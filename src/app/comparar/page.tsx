...
function identificarEnergia(carro: Carro): string {
  const texto =
    `${carro.motor} ${carro.desc}`.toLowerCase();
...
function transformarItensEmTexto(
  itens: string,
): string {
  return itens
    .split(",")
...
function AnimacaoCarrinho() {
  return (
    <div
      className="relative mx-auto h-28 w-full max-w-md overflow-hidden"
      aria-hidden="true"
    >
      <div className="carrinho-comparacao absolute bottom-1 left-0">
        <Image
          src="/carrinho-comparacao.png"
          alt=""
          width={460}
          height={250}
          priority
          className="h-auto w-52 scale-x-[-1] object-contain"
        />
      </div>

      <style jsx>{`
        @keyframes carrinho-comparacao {
          0% {
        ...
        .carrinho-comparacao {
          animation:
            carrinho-comparacao 4s linear infinite,
            movimento-suave 0.55s ease-in-out infinite;
...
function ConteudoComparacao() {
  ...
  let alturaImagemSelecionadaValue =
    "h-44";

  if (carrosSelecionados.length === 1) {
    alturaImagemSelecionadaValue =
      "h-72";
  } else if (
  ...
  let tamanhosImagemSelecionada =
    "(max-width: 1280px) 33vw, 360px";

  if (carrosSelecionados.length === 1) {
    tamanhosImagemSelecionada =
      ...
      0 ? (
        <section className="flex min-h-100 flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white px-5 py-12 text-center">
          ...
          <p className="mt-5 text-base text-slate-500">
            Marque Comparar nos cards do
            catálogo, ou comece com uma
            sugestão.
          </p>
